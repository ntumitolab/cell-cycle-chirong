...
def set_warnings():
    ...
    warnings.filterwarings('ignore', message='.end of stream*')
    warnings.filterwarings('ignore', message='.multiple of element size.*')
    
def get_dataset(opts, propper):
    if opts.mode == 'single':
        transform_signal = [eval(t) for t in opts.transform_signal]
        transform_signal.append(propper)
        ds = getattr(fnet.data, opts.class_dataset, opts.is_csv)(
            mode = opts.mode,
            path = opts.path_dataset,
...
def main():
    factor_yx = 1  # 0.108 um/px -> 0.29 um/px
    ...
    parser.add_argument('--mode', default='multi', help='cahnnel of input imagesLmu;ti or single.')
    parser.add_argument('--is_csv', default=True, help='Is path_dataset a csv file?')
    ...
    parser.add_argument('--path_dataset', type=str, help='path for constructing Dataset')
    ...
    if os.path.exists(opts.path_save_dir):
        ...
        return
    if opts.class_dataset in ['TiffDataset','TiffDataset2']:
    ...
    pdtarget = opts.path_model_dir[0].split('/')[1]
    print(pdtarget)
    pdtarget = pdtarget.split('_')[2] #to predict mitochondria or nucleus
    
    
    if opts.mode == 'multi':

        data_csv = pd.read_csv(opts.path_dataset)
        for idx in indices:
            ...
            a = data_csv.iloc[idx,0].split('/')

            save_dir = os.path.join(a[0],'\\',a[2],a[3],'val_pdmito',a[4],a[5],a[6],a[7])
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            
            save_name = save_dir + '\\' + a[8]
            for path_model_dir in opts.path_model_dir:
                ...
                tifffile.imsave(save_name, prediction.numpy()[0, ])
        ...
        if not opts.is_csv:  
        # if the path to the folder of data is given, the program runs this section.
        # this part haven't finished yet. Always use csv file
            for idx in indices:
                entry = get_prediction_entry(dataset, idx)
                data = [torch.unsqueeze(d, 0) for d in dataset[idx]]  # make batch of size 1
                signal = data[0]
            ...
            for idx in indices:
                ...
                data_csv = pd.read_csv(opts.path_dataset)
                a = data_csv.iloc[idx,0].split('/')
                save_dir = os.path.join(a[0],a[1],a[2],a[3],pdtarget,a[5])
                if not os.path.exists(save_dir):
                    os.makedirs(save_dir)
                ...
                for path_model_dir in opts.path_model_dir:
                    if (path_model_dir is not None) and (model is None or len(opts.path_model_dir) > 1):
                        ...
                        name_model = os.path.basename(path_model_dir)
                    prediction = model.predict(signal) if model is not None else None
                    tifffile.imsave(save_dir + '/' + a[-1], prediction.numpy()[0, ])
                    print('saved:' + save_dir + '/' + a[-1])
